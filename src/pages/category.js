...
import { getProductsbyCate } from "../instance/products";
import { getCate } from "../instance/cate";

const categoryPages = {
  async render(id) {
    const { data } = await getProductsbyCate(id);
    const dataCate = await getCate(id);
    return /*html */ `
    ${await header.render()}
